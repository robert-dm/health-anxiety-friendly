...
export function parseCookies(cookieHeader = "") {
  ...
  for (const part of cookieHeader.split(";")) {
    ...
    if (!rawName) continue;
    try { cookies[rawName] = decodeURIComponent(rawValue.join("=")); } catch { /* Ignore malformed cookies. */ }
  }
  return cookies;
}
...
export async function createSession(userId) {
  ...
  (await db.prepare("insert into sessions (id, user_id, token_hash, expires_at, created_at) values (?, ?, ?, ?, ?)").run(
    crypto.randomUUID(),
    userId,
    hashToken(token),
    expiresAt,
...
export async function getCurrentUser(request) {
  ...
  if (request.verifiedOwnerEmail && user.email.toLowerCase() === request.verifiedOwnerEmail && user.role !== 'admin') {
