...
export async function updateSuggestionStatus({ suggestionId, status, moderatorId }) {
  ...
  if (status === 'published') {
    ...
    if (proposal.target_type === 'professional') {
      ...
      const category = /psicolog|psiquiatr/.test(normalizedSpecialty) ? 'mental_health' : /cardiolog|clinica|dermatolog|gastroenterolog|neurolog|endocrinolog|otorrinolaringolog|oftalmolog|pediatr|traumatolog|ginecolog|urolog|odontolog/.test(normalizedSpecialty) ? 'medical' : 'other';
